...
### 14.37 Tunnel support (ngrok / Cloudflare) — preloader-only page, 2 root causes fixed
- User: *"when we are using services like cloudflare tunnel or ngrok the website is showing the pre loader continuously nothing else is showing but in the localhost it's working perfectly"*
- **Cause 1 — Next dev blocks cross-origin dev assets (the preloader itself).** Next 16 refuses chunk requests from any origin other than the hostname it started on. Verified with curl: a realistic tunnel script-tag request (tunnel `Referer` + `Sec-Fetch-*`) → **403**, while the SSR HTML loads fine — so the AdminLayout spinner is on screen but JS never loads, React never hydrates, nothing else ever appears. **Fixed**: `allowedDevOrigins` in **both** `next.config.ts` files — `*.ngrok-free.app`, `*.ngrok.app`, `*.ngrok.io`, `*.trycloudflare.com` (bare hostnames/wildcard labels only, no scheme or port, per Next docs; custom tunnel domains get one more line; ignored in production builds). Re-probe: tunnel Referer **403 → 200**, an unlisted foreign origin still 403 (selective whitelist, not a hole)
- **Cause 2 — the browser called `http://localhost:3001` directly.** `NEXT_PUBLIC_API_URL` was baked into the client bundle (`lib/api.ts`, login captcha fetch); on an HTTPS tunnel page that request dies three ways at once — mixed content block, CORS refusal, cross-site `SameSite=Lax` session cookie never sent. **Fixed**: `rewrites()` proxy in both configs (`/api/:path*` → API) + the browser halves of `lib/api.ts` now fetch **same-origin** (`typeof window === 'undefined' ? absolute : ''`), login page fetch relative. Server-side callers (middleware, server components, web's SSR pages) keep going direct — they run on this machine, localhost works behind any tunnel
- **E2E through the proxy — all PASS**: `GET :3002/api/settings/general` → 200, `GET :3002/api/captcha-config` → 200, fixture login via **:3002** → 200 with **both `Set-Cookie` headers surviving the rewrite**, then `GET :3002/api/auth/me` with that cookie jar → 200 (the exact same-origin round trip a tunneled browser performs); cleanup clean (1 pre-existing invite remains); web :3000 not running → its symmetric config untested at runtime
- tsc: admin green; web exactly the 2 pre-existing baseline errors (`buttons/page.tsx`, `Input.tsx`)
- Footnotes: (a) `allowedDevOrigins` is dev-only — production unaffected; (b) in a real deploy the env var now matters **server-side** (rewrite destination) — set `NEXT_PUBLIC_API_URL` (admin) / `API_URL` (web) to wherever the API actually lives; (c) if the Turnstile CAPTCHA ever appears behind a tunnel (2+ failed logins) and fails, that is Cloudflare hostname allowlisting in the Turnstile dashboard — config, not code
- **§14.37 uncommitted** (with §14.30–14.36)

**§14.37 follow-up — user retried ngrok, spinner still up: missing domain**
- User: *"I already tried with ngrok but still same issue"* — their tunnel (found via ngrok's local inspection API on :4040) is `https://subatomic-gentleman-snort.ngrok-free.**dev**` → `.ngrok-free.dev` was **not** in the wildcard list (only `.app`/`.io`/`.app`/`trycloudflare`) → chunks still 403, verified with their exact Referer: **403**. Added `*.ngrok-free.dev` to both configs
- Verified end-to-end **through their actual public tunnel URL**: chunk → **200 `application/javascript` (1.06 MB, real JS — no ngrok interstitial on assets, confirmed with browser-like headers and no skip header)**, page HTML → 200 (admin login), `/api/settings/general` proxy → **200**, foreign origin still 403
- ngrok free's own **browser-protection interstitial** appears once for HTML navigations ("Visit Site" click, cookie remembers) — expected on the free tier, unrelated to the spinner; script requests bypass it entirely
- User action: hard-refresh the tunnel URL → login page should render; **§14.37 still uncommitted** (2 next.configs, 2 api libs, login page, doc)

## §14.38 Security page: locations with country flags + rename + "this device" first
**User:** *last login should show the location; card renamed "Logged in devices"; "This device" pinned on top of the list; devices show location too; location always with the country flag.*

- **Where the IP comes from**: `getIp` reads `X-Forwarded-For` first, and the Next `/api` rewrite preserves it — verified E2E: login via :3002 with `XFF: 8.8.8.8` stores `8.8.8.8` (so ngrok's forwarded client IP reaches the API through the tunnel proxy). Plain localhost logins store `null` (no fake address).
- **Schema** (`migration 20260926005500_session_location`, applied via `migrate deploy` + `generate`): `Session.location` + `Session.countryCode`, `User.lastLoginLocation` + `User.lastLoginCountryCode` — all nullable.
- **New `packages/api/src/lib/geoLocation.ts`**: `resolveLocation(ip)` → ipwho.is (HTTPS, no key, tested reachable; ipapi.co was rate-limited), 2.5s timeout, in-memory cache 6h positive / 30min negative; private/reserved/unknown IPs short-circuit with no network call. `attachLoginLocation` — fire-and-forget after login/signup (never blocks the response), writes session + user fields; **overwrites user fields with null too**, otherwise a new private-IP login time would keep showing the *previous* login's place. `backfillSessionLocations` — on `GET /me/sessions`, rows with an IP but no stored location resolve once and persist (existing sessions fill in on first view).
- **API**: signup now captures `ipAddress`/`userAgent` like login (its sessions were invisible to device info); `/users/me/sessions` selects + returns `location`/`countryCode` per session and `lastLoginLocation`/`lastLoginCountryCode` at top level.
- **UI** (`settings/security/page.tsx`): `countryFlag(code)` builds the regional-indicator emoji from the ISO code at runtime (no glyph ever enters translations.ts); Last login card gains a **Location** InfoRow (`🇧🇩 Chattogram, Bangladesh`) before the IP row, falling back to the newest session's location when the user-level field is still empty; device rows show `· 🇧🇩 Chattogram, Bangladesh` before the IP; **This device pinned first** via a stable sort on a *display copy* — `data.sessions` stays untouched so `lastLoginAt`/`lastSession` keep pointing at the genuinely newest sign-in.
- **Copy**: `security.devicesTitle` → "Logged in devices" (লগইন করা ডিভাইস), plus de-trusted `settingsNav.securityDesc` and `security.description` in both languages; new key `security.locationLabel` ("Location" / অবস্থান).
- **Verification**: E2E **18/18** (proxy login+XFF → DB location `San Jose, United States`/`US`; payload fields; backfill re-resolve after wiping; localhost login → null location + cleared last-login location); tsc admin + api green; translations.ts **pure ASCII**; user's own live session already backfilled to `Chattogram, Bangladesh`.

## §14.39 Settings > Gateway page: SMS (Alpha.Net) + Payment (SSLCommerze) categories
**User:** *create the gateway pages with categories — SMS Gateway (Alpha.Net, using api.sms.net.bd send + balance APIs) and Payment Gateway (SSLCommerze, credentials design only; backend later); more gateways later.*

- **Page** (`settings/gateway/page.tsx`, was a 13-line placeholder): title/description + two category sections. **SMS Gateway → Alpha.Net**: enable toggle, API key (password input), **Check balance** button, **Send test SMS** block (phone + message, BD number normalization server-side), per-card Save. **Payment Gateway → SSLCommerze**: enable toggle, Store ID, Store password, **Sandbox mode** toggle, Save, and an info box stating the payment backend gets connected later (credentials store now, stay unused). "More gateways can be added here later." note under each category. Established patterns throughout: card/Section/input styles, switch-role toggles, spinner buttons, Toast, `t()` keys, no `<select>`.
- **Storage**: settings key `gateway` (permission `gateway.view`/`gateway.edit` already existed in `KEY_PERMISSIONS` + `permissions.ts`) — `{ sms: { alphaNet: { enabled, apiKey } }, payment: { sslcommerz: { enabled, storeId, storePassword, sandbox } } }`. The generic `PUT /api/settings` shallow-merges the top level, so saving one category's subtree never wipes the other (E2E-verified).
- **API** (`routes/settings.ts`, both `requireKeyEdit('gateway')`): `POST /settings/gateways/sms/alpha/balance` → proxies `GET api.sms.net.bd/user/balance/?api_key=…`; `POST /settings/gateways/sms/alpha/send` → multipart `api_key`/`msg`/`to` to `api.sms.net.bd/sendsms`, rate-limited 5/min (a retry loop can't drain the balance). Alpha answers **HTTP 200 even on errors**, so success is decided from the body (`error` truthy / `success:false` / `status:'error'`) and the gateway's own `msg` is proxied through (e.g. "Authorization required"). 15s timeout via the `AbortController`+`setTimeout` pattern from `geoLocation.ts`; unreachable gateway → 502. Phone accepts `01…` or `8801…`/`+8801…` → normalized to 13-digit `8801…`; message capped at 1000 chars.
- **Copy**: en + bn keys `gateway.*` (30 keys: categories, Alpha/SSLCommerze cards, fields, buttons, test block, sandbox, pending-backend note) + stale `gateway.description` and `settingsNav.gatewayDesc` rewritten ("SMS and payment gateway connections"). translations.ts verified **pure ASCII** (0 non-ASCII chars), key parity en+bn confirmed; tsc admin + api green.
- **E2E 16/16 ALL GREEN** (`/tmp/opencode/e2e-gateway.sh`): full save/load, partial save preserves sibling subtree, balance with fake key → 400 + proxied "Authorization required", missing key → 400 `API_KEY_REQUIRED` (no network call), invalid phone → 400 `INVALID_PHONE`, send with fake key → gateway error text, no auth → 401, cleanup resets to empty defaults. (Probed Alpha live first: errors return `{"error":405,"msg":"Authorization required"}` with HTTP 200.)
- **Uncommitted** (alongside §14.38): gateway page, translations.ts, routes/settings.ts, done-so-far.md.

**§14.39 follow-up — object-storage-style toggles + inline balance**
- User: *enable/disable + hide/unhide same as object storage; balance shows on page load, not a response toast; icon-only refresh button beside it.*
- **Enable cards** (per category, copied from `settings/object-storage/page.tsx`): heading + description left, toggle right — and the config card (keys, balance, test-send / SSL credentials, sandbox) **renders only while enabled**; Save stays visible below either way.
- **Hide/unhide**: `Show secret keys` checkbox under the inputs (object-storage pattern), switching input type text/password — one per card (`showAlphaKey` for the API key, `showSslSecrets` for Store ID + Store password); secrets use the monospace input style + focus/blur border transitions like R2.
- **Balance inline**: no toast. Rendered as `Balance: BDT …` (or error text in red) next to an **icon-only refresh button** (32px, `aria-label`/`title` = "Refresh balance", spins while loading, keeps the last value visible dimmed during refresh). Auto-fetch on page load when the gateway is enabled with a saved key; also after Save and when toggling on — never for a missing key.
- **Translations**: +8 keys (`enableAlpha`, `enableAlphaDesc`, `enableSsl`, `enableSslDesc`, `showKeys`, `balanceLabel`, `balanceLoading`, `refreshBalance`), −2 obsolete (`enableLabel`, `checkBalance`) — en and bn kept at parity (36 gateway keys ×1 per block), file still **pure ASCII**. Fix from earlier this session: `settingsNav.gatewayDesc` had been swapped into the en block — corrected (en: "SMS and payment gateway connections", bn escaped).
- Verified: tsc admin GREEN, gateway page compiles 200 (authed), key-parity + ASCII gates scripted.

**§14.39 follow-up 2 — send-test removed**
- User: *remove the send test — the balance already proves the connection, no test SMS / test connection anywhere.*
- Page: test-send form + footer button, its states/handlers/`outlineButtonStyle`/`FormEvent` import removed — footer is **[Save]** only; Alpha card now ends at the balance row. API: `POST /settings/gateways/sms/alpha/send` deleted (verified **404**), `normalizeBdPhone` + the settings.ts `rateLimit` import dropped; balance endpoint untouched (verified live: fake key → 400 "Authorization required").
- Translations: 8 keys removed from en+bn (`testTitle`, `testTo`, `testToPlaceholder`, `testMessage`, `testMessagePlaceholder`, `sendTest`, `sentSuccess`, `sentFailed`) — parity intact, pure ASCII. tsc admin + api GREEN, page compiles 200.

**§14.39 follow-up 3 — "Show secret keys" checkbox removed everywhere**
- User: remove it from the object storage page **and** the gateway page (confirmed scope). Secret inputs on both pages are now permanently `type="password"` — no show/hide toggle.
- Dropped: `showAlphaKey`/`showSslSecrets` states + both checkbox blocks (gateway), `showSecrets` state + checkbox block (object storage); translation keys `gateway.showKeys` and the pre-existing dead `objectStorage.showSecrets` removed from en+bn — parity intact, pure ASCII. tsc admin GREEN; both pages compile 200.
